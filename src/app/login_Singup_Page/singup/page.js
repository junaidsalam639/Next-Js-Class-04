...
export default function Singup() {
    ...
    const onClickBtn = async () => {
        ...
        if (email != '' || password != '') {
            ...
            if (result) {
                router.replace('/login_Singup_Page/login');
            }
            else {
                alert(error.message)
            }
            ...
            <section className="mt-3 text-gray-600 body-font relative">
                <div className="container px-5 mx-auto flex">
                    <div className=" bg-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
                        <h2 className="text-gray-900 text-lg mb-1 font-medium title-font">Singup</h2>
                        ...
                        <button disabled={loading} onClick={onClickBtn} className="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg">Singup</button>
                        <p className="text-xs text-gray-500 mt-3">Chicharrones blog helvetica normcore iceland.
                        <Link href={'/login_Singup_Page/login'} className="text-indigo-600">Login?</Link></p>
                    </div>
                </div>
            </section>
        </main>
